...
function validationRules() {
  return {
    ...
    email: (inputProps) => {
      const validationRule = /^[_a-z0-9-]+(.[_a-z0-9-]+)*@[a-z0-9-]+(.[a-z0-9-]+)*(.[a-z]{2,4})$/;
      const inputValue = inputProps.value;
      const inputName = inputProps.name;
      const isInputValid = validationRule.test(inputValue);
    ...
    terminos: (inputProps) => {
      const inputValue = inputProps.checked;
      const inputName = inputProps.name;
...
function validateForm() {
  const verifyInputName = {
    ...
    terminos: validationRules().terminos
  };
  ...
  return validationState.size === 0
    ? swal(
        "¡Datos recibidos!",
        "¡En cuestión de minutos te responderemos!",
        "success"
      )
    : "";
}
